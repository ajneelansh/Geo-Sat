...
export default function Maintool() {
  ...
  const handleUpdateData = () => {
    setIsLoading(true)
    setTimeout(() => {
      const newMetadata = generateSceneMetadata(coordinates.split(',')[0], coordinates.split(',')[1])
      setMetadata({
        ...
        <div className="container mx-auto max-w-6xl">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="bg-gray-800 p-6 rounded-lg shadow-lg">
              ...
              <div className="mt-4">
                <label htmlFor="coordinates" className="block text-sm font-medium text-gray-300">Selected Coordinates:</label>
                <input
                  type="text"
                  id="coordinates"
                  value={coordinates}
            ...
            <div className="bg-gray-800 p-6 rounded-lg shadow-lg">
              ...
              <div className="grid grid-cols-3 gap-2 mb-4">
                {Array(9).fill(null).map((_, index) => (
                  <div key={index} className="aspect-square bg-gray-700 flex items-center justify-center text-sm">
                    {selectedBand === 'natural' ? '0.15, 0.18, 0.12' : selectedBand === 'ndvi' ? '0.45' : '0.10'}
                  </div>
                ))}
              </div>
              <h3 className="text-lg font-semibold mb-2">Spectral Signature</h3>
              <Bar
                data={{
